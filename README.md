...
or **PostgreSQL** (`postgres://`) — the same 87 conformance checks green on all
...
- **Contract-first.** Behavior is defined by [openapi.yaml](./openapi.yaml)
  ...
  (87 checks) that the Go binary greens. A language-agnostic suite means a future
...
`inc` adds to a column on every matching event. A number is a literal (`{ "inc":
{ "moves": 1 } }` counts events); a `$.` string is a payload path, so a total is
maintained by the fold rather than by the app:

```jsonc
{ "op": "upsert", "inc": { "on_hand": "$.quantity" } }   // negatives included
```
...
A rebuild reports the same gap as `skipped`. Events the current rules cannot
fold are counted and stepped over rather than abandoning the whole view — a
rebuild is the repair mechanism, so one bad event must not cost every good one.

`key` decides which row a rule writes. Omitted, it is the stream the event
landed on — one stream, one row, as it has always been. Given a `$.` path, the
row is identified by the payload instead, so a fold can maintain a total that
spans streams:

```jsonc
{
  "StockMoved":      { "op": "upsert", "key": "$.sku", "inc": { "on_hand": "$.quantity" } },
...
two kinds of identity in one table.

A key must resolve to a **non-empty string**, or a **whole number below 2^53**
(written as text: `42`, never `42.0`). Everything else fails the fold — `null`,
an absent path, `""`, a fraction, a number at or past 2^53, an object, an array.
The 2^53 bound is not arbitrary: a payload's numbers arrive as float64, and past
that an integer is no longer the one that was sent, so two ids would quietly
become one row. Inside the bound a numeric key is its double value, so `1` and
`1.0` are the same identity; an id needing more fidelity than a double belongs
in a string. Within a projection every rule carries a key
...
just conformance     # the behavior lock: Go must green all 87 checks
just conformance-pg  # the same 87 checks against a real PostgreSQL
