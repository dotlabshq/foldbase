...
| `conformance/` | `run.mjs` (64 checks) + `realtime.mjs` (13 checks) — boot a server binary and assert the contract over HTTP. The behavior lock. |
...
just conformance        # the 64 contract checks (embedded SQLite)
just conformance-pg     # the same 64 against a real PostgreSQL
just conformance-libsql # the same 64 against a real Turso/sqld
just test-all        # conformance + realtime + TS & Python client + subscribe smokes
just dev-web         # taskboard demo UI → http://localhost:4000
```

Current status: the Go backend greens **all 64 conformance checks on all three
